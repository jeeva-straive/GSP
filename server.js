...
function requestWithMethod(rawUrl, method = "HEAD", timeoutMs = 10000, maxRedirects = 5) {
...
app.post("/analyze-url", async (req, res) => {
  const { url } = req.body || {};
  if (!isValidUrl(url)) {
    return res.status(400).json({ success: false, valid: false, error: "Invalid URL" });
  }
  ...
  return res.json({
    ...
    contentType,
    server: (result.headers && (result.headers.server || result.headers.Server)) || "",
  ...
  try {
    ...
    try {
      ...
      for (const s of cookieSelectors) {
        ...
        if (el) { await el.click({ timeout: 1000 }).catch(()=>{}); break; }
      }
    } catch {}
    const data = await page.evaluate(() => {
      const sel = ['main','article','.content','.main-content','.entry-content','#content','.post-content','[role="main"]','body'];
      const pick = () => { for (const s of sel) { const el = document.querySelector(s); if (el && (el.textContent||'').trim().length>50) return el; } return document.body; };
      const main = pick();
      ...
      const isFilterZone = (el) => {
        const badSelectors = [
          ...
          '[role="search"]', '[role="navigation"]', '.breadcrumb', '.pagination', '.pager'
        ];
        let node = el; let depth = 0;
        while (node && depth < 6) {
          if (badSelectors.some(sel => node.matches && node.matches(sel))) return true;
      ...
      const isFilterHref = (href) => {
        try {
          ...
          if (keys.includes('filter') || keys.includes('facet') || keys.includes('refine') || keys.includes('search') || keys.includes('q=')) return true;
          return false;
        } catch { return false; }
      };
      ...
      const videos = Array.from(main.querySelectorAll('video, iframe[src*="youtube"], iframe[src*="vimeo"]'));
      const forms = Array.from(main.querySelectorAll('form'));
      const headings = Array.from(main.querySelectorAll('h1,h2,h3,h4,h5,h6')).map(h=>({level:h.tagName.toLowerCase(),text:t(h)}));
      const hasLogin = !!document.querySelector('input[type="password"], [name*="password" i]');
      const frameworkSignals = { hasAngular: !!(window).ng || !!document.querySelector('[ng-version]'), hasReact: !!(window).React || !!document.querySelector('[data-reactroot], [data-reactid]'), hasVue: !!(window).Vue || !!document.querySelector('[id^="__nuxt"], [data-v-]') };
      ...
      if (linkEls.length>100 && wordCount<400) contentType='directory/listing';

      const hasHyperlinks = docLinks.length > 0;
      const hasDirectContent = wordCount > 50;
      const hasKeyValue = (() => {
        const hasDL = !!document.querySelector('dl dt, dl dd');
        const kvTable = Array.from(document.querySelectorAll('table')).some((tb) => {
          const rows = Array.from(tb.querySelectorAll('tr'));
          let score = 0;
          for (const tr of rows) {
            const ths = tr.querySelectorAll('th');
            const tds = tr.querySelectorAll('td');
            if ((ths.length === 1 && tds.length === 1) || tds.length === 2) score++;
          }
      ...
      const baseForList = linkEls.length ? linkEls : (allLinks.length ? allLinks : docLinks);
      const hrefs = Array.from(new Set(baseForList.map(a => a.href))).slice(0, 50);

      return { title: t(document.querySelector('title')), pageHeading: t(document.querySelector('h1')), wordCount, linkCount: baseForList.length, imageCount: images.length, tableCount: tables.length, videoCount: videos.length, formCount: forms.length, headings, frameworkSignals, contentType, url: location.href, types: { hyperlinks: hasHyperlinks, directContent: hasDirectContent, keyValue: hasKeyValue }, linksList: hrefs };
    });
    await browser.close();
...
app.post('/jobs/run', async (req, res) => {
  try {
    ...
    (async () => {
      Runs.set(runId, Object.assign({}, Runs.get(runId), { status: 'running' }));
      try {
        const { rows } = await runJob(job);
        Runs.set(runId, { status: 'done', startedAt: Runs.get(runId).startedAt, finishedAt: Date.now(), rows });
        const outDir = path.join(__dirname, 'runs', runId);
        try { fs.mkdirSync(outDir, { recursive: true }); } catch {}
        const cols = Array.from(new Set(rows.flatMap(r => Object.keys(r || {}))));
        const esc = (s) => '"' + String(s ?? '').replace(/"/g, '""') + '"';
        const csv = [cols.join(',')].concat(rows.map(r => cols.map(c => esc(r[c])).join(','))).join('\n');
        try { fs.writeFileSync(path.join(outDir, 'result.csv'), csv, 'utf8'); } catch {}
        try { fs.writeFileSync(path.join(outDir, 'result.json'), JSON.stringify(rows, null, 2), 'utf8'); } catch {}
      } catch (e) {
        Runs.set(runId, { status: 'failed', startedAt: Runs.get(runId).startedAt, finishedAt: Date.now(), rows: [], error: String(e && e.message || e) });
      }
    })();
...
app.get('/jobs/:id/status', (req, res) => {
  ...
  res.json({ success: true, status: run.status, startedAt: run.startedAt, finishedAt: run.finishedAt || null, rows: (run.rows || []).length, error: run.error || null });
});
...
app.post("/select-strategy", async (req, res) => {
  ...
  try {
    ...
    const signals = await page.evaluate(() => {
      ...
      const hasCF = /cloudflare|attention required|captcha/i.test(document.body.innerText||'') || !!document.querySelector('#cf-please-wait');
      const hasBlock = /access denied|forbidden|not authorized|blocked/i.test(document.body.innerText||'');
      const spa = !!(window).ng || !!(window).React || !!(window).Vue || !!document.querySelector('[ng-version], [data-reactroot], [id^="__nuxt"], [data-v-]');
    ...
    if (status === 403 || status === 401 || status === 429 || signals.hasCF || signals.hasBlock) strategy = 'Blocked/WAF';
    else if (signals.spa || signals.jsHeavy || textAfter > signals.textLen * 1.5) strategy = 'JS/SPA/Dynamic';
    await browser.close();
    res.json({ success: true, data: { status, strategy, signals: { ...signals, textAfter } } });
  } catch (e) {
    await browser.close();
    res.status(500).json({ success: false, error: e.message });
  }
...
app.post("/extract", async (req, res) => {
  ...
  try {
    ...
    await page.goto(url, { waitUntil: 'domcontentloaded', timeout: 60000, referer: origin });
    try { await page.waitForLoadState('networkidle', { timeout: 15000 }); } catch {}
    try {
      const cookieSelectors = [
        '#onetrust-accept-btn-handler',
    ...
    const getEligibleCount = async () => {
      return await page.evaluate(() => {
        ...
        const badSelectors = [
          'form','nav','header','footer','aside','[role="search"]','[role="navigation"]','.breadcrumb','.pagination','.pager','.filters','.filter','.facets','.facet','.refine','.search','.toolbar','.sidebar'
        ];
        const isFilterZone = (el) => { let node = el; while (node) { if (badSelectors.some(sel => node.matches && node.matches(sel))) return true; node = node.parentElement; } return false; };
        const isFilterHref = (href) => { try { const u = new URL(href, location.href); const keys = Array.from(u.searchParams.keys()).join(',').toLowerCase(); return keys.includes('filter')||keys.includes('facet')||keys.includes('refine')||keys.includes('search')||keys.includes('q='); } catch { return false; } };
    ...
    while (pagesVisited < maxAdvances) {
      ...
      const selectors = [
        ...
        'a[rel="next"]', 'a[aria-label*="next" i]', 'button:has-text("Next")', 'a:has-text("Next")'
      ];
      for (const s of selectors) {
        try {
          const el = await page.$(s);
          if (el) { await el.click({ timeout: 2000 }); advanced = true; break; }
        } catch {}
      }
      ...
      if (!advanced) {
        try {
          const nextHref = await page.evaluate(() => {
            const a = document.querySelector('a[rel="next"], a[aria-label*="next" i]');
            return a ? a.getAttribute('href') : '';
          });
          if (nextHref) { await page.goto(nextHref, { waitUntil: 'domcontentloaded', timeout: 30000 }); advanced = true; }
        } catch {}
      }
      ...
      if (!advanced) {
        try {
          await page.evaluate(async () => {
            const delay = (ms) => new Promise(r => setTimeout(r, ms));
            for (let y = 0; y < (document.body?.scrollHeight || 0); y += 800) { window.scrollTo(0, y); await delay(150); }
            window.scrollTo(0, document.body?.scrollHeight || 0);
          });
          advanced = true;
        } catch {}
    ...
    const data = await page.evaluate(() => {
      ...
      const isFilterZone = (el) => {
        const badSelectors = [
          'form', 'nav', 'header', 'footer', 'aside',
          '[role="search"]', '[role="navigation"]', '.breadcrumb', '.pagination', '.pager',
          '.filters', '.filter', '.facets', '.facet', '.refine', '.search', '.toolbar', '.sidebar'
        ];
        let node = el;
        while (node) {
          if (badSelectors.some(sel => node.matches && node.matches(sel))) return true;
      ...
      let links = allLinks
        ...
        .map(a=>({ href:a.href, text:t(a), titleAttr:a.getAttribute('title')||'' }));
      if (!links.length) {
        links = allLinks
          .filter(a => {
            const href = a.getAttribute('href') || '';
      ...
      const images = Array.from(main.querySelectorAll('img')).map(i=>({ src:i.src, alt:i.alt||'' }));
      const tables = Array.from(main.querySelectorAll('table')).map(tb=>tb.outerHTML);
      const lists = Array.from(main.querySelectorAll('ul,ol')).map(list=>({ type:list.tagName.toLowerCase(), items:Array.from(list.querySelectorAll('li')).map(li=>t(li)) }));
      const totalLinks = links.length;
      const linksWithText = links.filter(l=>l.text.length>0).length;
      const linksWithTitleAttr = links.filter(l=>l.titleAttr && l.titleAttr.length>0).length;
      const maxTextLen = links.reduce((m,l)=>Math.max(m,l.text.length),1);
      const linksScored = links.map(l=>({ ...l, textCoveragePct: Math.round((l.text.length/Math.max(maxTextLen,1))*100) }));
      return { url: location.href, title: t(document.querySelector('title')), pageHeading: t(document.querySelector('h1')), headings, links: linksScored, images, tables, lists, stats: { totalLinks, linksWithText, linksWithTitleAttr, percentWithText: totalLinks? Math.round((linksWithText/totalLinks)*100):0, percentWithTitleAttr: totalLinks? Math.round((linksWithTitleAttr/totalLinks)*100):0 } };
    });
    await browser.close();
    res.json({ success: true, data });
  } catch (e) {
...
app.post('/extract-one', async (req, res) => {
  ...
  try {
    ...
    const contentType = resp && typeof resp.headers === 'function' ? (resp.headers()['content-type'] || '') : '';
    let content = null;
    if (/text\/html|application\/xhtml\+xml/i.test(contentType) || !contentType) {
      content = await page.evaluate(() => {
        const sels = ['main','article','.content','.main-content','.entry-content','#content','.post-content','[role="main"]','body'];
        let main=null; for (const s of sels){ const el=document.querySelector(s); if(el && (el.textContent||'').trim().length>50){ main=el; break; }}; if(!main) main=document.body;
        const t=(e)=> (e? (e.textContent||''): '').trim();
        ...
        const images = Array.from(main.querySelectorAll('img')).map(i=>({ src:i.src, alt:i.alt||'' }));
        const tables = Array.from(main.querySelectorAll('table')).map(tb=>tb.outerHTML);
        return { title, pageHeading, textContent, headings, images, tables, html: main.innerHTML };
      });
    }
    await browser.close();
...
app.post('/extract-keyvalue', async (req, res) => {
  ...
  try {
    ...
    const result = await page.evaluate(() => {
      ...
      const tables = Array.from(main.querySelectorAll('table'));
      const tableKeyValues = tables.map((tb) => {
        const rows = Array.from(tb.querySelectorAll('tr'));
        const list = [] ;
        for (const tr of rows) {
          const ths = Array.from(tr.querySelectorAll('th')).map(t);
          const tds = Array.from(tr.querySelectorAll('td')).map(t);
          if (ths.length === 1 && tds.length === 1) list.push({ key: ths[0], value: tds[0] });
          else if (tds.length === 2) list.push({ key: tds[0], value: tds[1] });
        }
        return list.filter(kv => kv.key || kv.value);
      }).filter(arr => arr.length);

      const dlKeyValues = Array.from(main.querySelectorAll('dl')).map((dl) => {
        const items= [];
        let currentKey = '';
        Array.from(dl.children).forEach((node) => {
          if (node.tagName && node.tagName.toLowerCase() === 'dt') currentKey = t(node);
          else if (node.tagName && node.tagName.toLowerCase() === 'dd') items.push({ key: currentKey, value: t(node) });
        });
...
app.post('/scrape-with-strategy', async (req, res) => {
  ...
  try {
    if (s.includes('blocked')) {
      ...
      const get = (!head || (head.status||0)>=400) ? await requestWithMethod(url, 'GET') : null;
      await browser.close();
      return res.json({ success: true, data: { strategy: 'Blocked/WAF', note: 'Access appears restricted. Consider authentication, proxy, or manual headers.', head, get } });
    }
    ...
    if (s.includes('js') || s.includes('dynamic') || s.includes('spa')) {
      ...
      try {
        await page.evaluate(async () => {
          const delay = (ms) => new Promise(r=>setTimeout(r, ms));
          for (let y=0; y<document.body.scrollHeight; y+=600) { window.scrollTo(0, y); await delay(200); }
    ...
    const content = await page.evaluate(() => {
      ...
      const textContent = t(main);
      const headings = Array.from(main.querySelectorAll('h1,h2,h3,h4,h5,h6')).map((h)=>({ level:h.tagName.toLowerCase(), text:t(h) }));
      const links = Array.from(main.querySelectorAll('a[href]')).map((a)=>({ href:a.href, text:t(a), titleAttr:a.getAttribute('title')||'' }));
      const images = Array.from(main.querySelectorAll('img')).map((i)=>({ src:i.src, alt:i.alt||'' }));
      const tables = Array.from(main.querySelectorAll('table')).map((tb)=>tb.outerHTML);
      return { title, pageHeading, textContent, headings, links, images, tables, html: main.innerHTML };
    });
    await browser.close();
...
app.post("/extract-links", async (req, res) => {
  const { links = [], limit = 20, waitTime = 500, concurrency = 3 } = req.body || {};
  if (!Array.isArray(links) || links.length === 0) {
    return res.status(400).json({ success: false, error: "links[] required" });
  }
  const list = links.filter((u) => {
  ...
  const maxWorkers = Math.max(1, Math.min(Number(concurrency) || 1, 10, list.length));
  ...
  const worker = async (workerId) => {
    ...
    try {
      while (true) {
        ...
        const href = list[idx];
        try {
          const resp = await page.goto(href, { waitUntil: 'domcontentloaded', timeout: 60000 });
          try { await page.waitForLoadState('networkidle', { timeout: 5000 }); } catch {}
          const status = resp ? resp.status() : 0;
          const content = await page.evaluate(() => {
            const sels = ['main','article','.content','.main-content','.entry-content','#content','.post-content','[role="main"]','body'];
  ...
  try {
    ...
    await browser.close();
    res.json({ success: true, count: results.length, concurrency: maxWorkers, data: results });
  } catch (e) {
    await browser.close();
    res.status(500).json({ success: false, error: e.message });
  }
...
app.post("/scrape", async (req, res) => {
  ...
  try {
    ...
    const awardLinks = await page.evaluate(() => {
      ...
      return links
        .filter(link => link.href && !link.href.startsWith('#') && !link.href.includes('mailto:'))
        .map(link => ({
          url: link.href,
  ...
  try {
    ...
    try {
      const revealSelectors = [
        'a[href="#show-filters"]', '#show-filters',
        'button:has-text("Show Filters")', 'a:has-text("Show Filters")',
        'button:has-text("Filters")', 'a:has-text("Filters")',
        '.filters-toggle', '.filter-toggle', '[data-toggle="collapse"][href*="filter" i]', '[data-bs-toggle="collapse"][href*="filter" i]'
      ];
      ...
      await page.evaluate(() => {
        const maybeClick = (el) => { try { (el instanceof HTMLElement) && el.click(); } catch {} };
        document.querySelectorAll('[aria-expanded="false"][aria-controls]').forEach((btn) => {
          const ctrl = btn && btn.getAttribute ? (btn.getAttribute('aria-controls') || '') : '';
          if (/filter|facet|refine|option|panel/i.test(ctrl||'')) maybeClick(btn);
        });
        // Also expand common accordion controls
        document.querySelectorAll('[data-bs-toggle="collapse"],[data-toggle="collapse"]').forEach((btn) => maybeClick(btn));
      });
    ...
    const data = await page.evaluate(() => {
      ...
      const getLabel = (el) => {
        ...
        let n = el; while (n && n!==document.body) { if (n.tagName && n.tagName.toLowerCase()==='label') return t(n); n = n.parentElement; }
      ...
      const collectFields = (rootEl) => {
        ...
        const add = (name, field) => {
          ...
          if (field.kind==='checkbox' || field.kind==='radio') {
            const existing = byName.get(name) || { kind: field.kind, name, label: field.label, options: [] };
            const opts = existing.options || [];
            opts.push({ value: field.optionValue || field.value || 'on', label: field.optionLabel || field.label || (field.value||'') });
            existing.options = opts; existing.label = existing.label || field.label; byName.set(name, existing);
          } else if (field.kind==='select') {
            byName.set(name, { kind: 'select', name, label: field.label, options: field.options || [] });
          } else if (field.kind==='textarea') {
            byName.set(name, { kind: 'textarea', name, label: field.label, value: field.value || '' });
          } else {
            byName.set(name, { kind: field.kind || 'text', name, label: field.label, value: field.value || '' });
          }
        };
        const els = Array.from(rootEl.querySelectorAll('input[name], select[name], textarea[name]'));
        for (const el of els) {
          const tag = el.tagName.toLowerCase();
          let name = (el.getAttribute('name') || el.getAttribute('id') || '').trim();
          ...
          if (type === 'checkbox' || type === 'radio') {
            ...
            const optLabel = labelText || (el.getAttribute && (el.getAttribute('title')||'')) || (el.value||'');
            if (!(optLabel||'').trim().length) continue;
            add(name, { kind: type, name, label: labelText, value: (el && el.value) ? el.value : 'on', optionValue: (el && el.value) ? el.value : 'on', optionLabel: optLabel });
          } else if (type === 'select') {
            ...
            if (!isVisible(el)) continue;
            add(name, { kind: 'textarea', name, label: labelText, value: (el && el.value) ? el.value : '' });
          } else {
            if (!isVisible(el)) continue;
            const placeholder = el.getAttribute && el.getAttribute('placeholder') || '';
            if (!labelText && !placeholder) continue;
            add(name, { kind: type, name, label: labelText, value: (el && el.value) ? el.value : '' });
          }
        }
        return byName;
      };
      ...
      const forms = Array.from(document.querySelectorAll('form')).map((f) => {
        ...
        const byName = new Map();
        for (const el of Array.from(f.querySelectorAll('input[name], select[name], textarea[name]'))) {
          let name = (el.getAttribute('name') || el.getAttribute('id') || '').trim(); if (!name) continue;
          const kind = el.tagName.toLowerCase()==='input' ? ((el.getAttribute('type')||'text')+'').toLowerCase() : el.tagName.toLowerCase();
          if (kind==='password' || kind==='hidden') continue;
          if (el.hasAttribute && el.hasAttribute('disabled')) continue;
          if (kind==='checkbox' || kind==='radio') {
            if (!isControlVisible(el)) continue;
            const lbl = getLabel(el) || (el.getAttribute && (el.getAttribute('title')||'')) || (el.value||'');
            if (!(lbl||'').trim().length) continue;
            const opt = { value: el.value || 'on', label: lbl };
            const group = byName.get(name) || { kind, name, label:getLabel(el.parentElement||el), options:[] };
            group.options.push(opt); byName.set(name, group);
          } else if (kind==='select') {
            if (!isVisible(el)) continue;
            const options = Array.from(el.options||[])
              ...
              .filter(o=> (o.label||'').trim().length>0 || (o.value||'').trim().length>0);
            if (options.length < 1) continue;
            byName.set(name, { kind:'select', name, label:getLabel(el), options });
          } else {
            if (!isVisible(el)) continue;
      ...
      const candidates = Array.from(document.querySelectorAll('#filters, [id*="filter" i], [class*="filter" i], [aria-label*="filter" i]'));
      let scope = candidates.sort((a,b)=>{
        const ac = a.querySelectorAll('input[name], select[name], textarea[name]').length;
        const bc = b.querySelectorAll('input[name], select[name], textarea[name]').length;
        return bc - ac;
      })[0] || document.body;
      try {
        ...
        if (extraFields.length) {
          forms.push({ method: 'GET', action: '#filters', fields: extraFields });
        }
      } catch {}

      // Infer anchor-based query parameters used as filters
      const keysHint = ['filter','filters','facet','facets','refine','refinement','fq','category','type','tag','topic','discipline','program','sort','order','page_size','per_page','q','from','to','year'];
      ...
      for (const a of Array.from(scope.querySelectorAll('a[href]'))) {
        ...
        try {
          ...
          for (const [k,v] of u.searchParams.entries()) {
            ...
            if (keysHint.some(h=>kl.includes(h)) || u.searchParams.getAll(k).length>1) {
      ...
      const anchorParams = Array.from(paramMap.entries()).map(([key, set]) => ({ key, options: Array.from(set).slice(0,100).map(v=>({ value:v, label:v })) }));
...
app.post('/build-filtered-url', async (req, res) => {
  try {
    ...
    if (!isValidUrl(url) || typeof params !== 'object') {
      return res.status(400).json({ success:false, error:'url and params required' });
    }
    const u = new URL(url);
    ...
    for (const [k,v] of Object.entries(params)) {
      ...
      if (Array.isArray(v)) { for (const item of v) { if (item!==undefined && item!==null && String(item).length) u.searchParams.append(k, String(item)); } }
  ...
  try {
    ...
    if (!results.some(r => r.isParent)) {
      try {
        ...
        const inferredParentUrl = urlObj.origin + urlObj.pathname.substring(0, urlObj.pathname.lastIndexOf('/') + 1);
        
        await page.goto(inferredParentUrl, { waitUntil: 'domcontentloaded', timeout: 30000 });
        try { await page.waitForLoadState('networkidle', { timeout: 5000 }); } catch {}
    ...
    if (parentResult) {
      ...
      const siblings = parentResult.data.links.filter(link => 
        link.href !== url && 
        new URL(link.href).origin === urlObj.origin
      );
      
      // Identify which link is our target
      const targetLink = parentResult.data.links.find(link => link.href === url);
...
app.post('/selector-siblings', async (req, res) => {
  ...
  try {
    let parentUrl = directParentUrl && isValidUrl(directParentUrl) ? directParentUrl : null;
      ...
      if (parentSelector) {
        parentUrl = await page.evaluate((sel) => {
          try {
            ...
            if (!root) return null;
            const a = (root.tagName && root.tagName.toLowerCase() === 'a')
              ? root
              : (root.closest && root.closest('a[href]')) || root.querySelector('a[href]');
            if (!a) return null;
      ...
      if (!parentUrl) {
        parentUrl = await page.evaluate(() => {
          ...
          const fromCandidates = (els) => {
            for (const a of els) {
              try { return new URL(a.href, document.location.href).href; } catch {}
            }
            return null;
          };
          ...
          const breadcrumbAnchor = fromCandidates([
            ...
            (() => { const nav = document.querySelector('[aria-label*="breadcrumb" i]'); return nav ? nav.querySelector('a[href]') : null; })(),
            first('a[rel="up"]'),
            first('a[rel="parent"]'),
          ].filter(Boolean));
    ...
    if (!parentUrl) {
      await browser.close();
      return res.json({ success: false, error: 'Could not locate a parent link via selectors on the page' });
    }
    ...
    const data = await page.evaluate((containerSel, linkSel, targetUrl, doAutoScope) => {
      const t = (e) => (e ? (e.textContent || '').trim() : '');
      const pickMain = () => {
        const sels = ['main','article','.content','.main-content','.entry-content','#content','.post-content','[role="main"]','body'];
        for (const s of sels) { const el = document.querySelector(s); if (el && (el.textContent||'').trim().length > 50) return el; }
        return document.body;
      };
      const toAbs = (a) => { try { return new URL(a.href, document.location.href).href; } catch { return null; } };
      let scope = (containerSel ? document.querySelector(containerSel) : null) || pickMain();

      const badSelectors = [
        'form','nav','header','footer','aside','[role="search"]','[role="navigation"]','.breadcrumb','.pagination','.pager','.filters','.filter','.facets','.facet','.refine','.search','.toolbar','.sidebar'
      ];
      const isFilterZone = (el) => { let n = el; while (n) { if (badSelectors.some(sel => n.matches && n.matches(sel))) return true; n = n.parentElement; } return false; };

      let linkNodes = Array.from(scope.querySelectorAll(linkSel || 'a[href]'));
      ...
      const seen = new Set();
      const dedup = [];
      for (const l of links) { if (!seen.has(l.href)) { seen.add(l.href); dedup.push(l); } }

      let targetLink = dedup.find(l => l.href === targetUrl) || null;

      // Optional: auto-scope to the same repeated container as the target link
      if (doAutoScope) {
        const targetAnchor = Array.from(document.querySelectorAll('a[href]')).find(a => {
          try { return new URL(a.href, document.location.href).href === targetUrl; } catch { return false; }
        }) || null;
        if (targetAnchor) {
          const hasItemLink = (el) => !!(el && (el.querySelector ? (el.querySelector(linkSel || 'a[href]')) : null));
          let node = targetAnchor;
          let container = null;
          while (node && node !== document.body) {
            const p = node.parentElement;
            if (!p) break;
            const siblings = Array.from(p.children).filter(ch => ch.tagName === node.tagName && ch.className === node.className);
            const valid = siblings.filter(hasItemLink);
            if (valid.length >= 1 && hasItemLink(node)) { container = node; break; }
            node = p;
          }
          if (container && container.parentElement) {
            const groupItems = Array.from(container.parentElement.children).filter(ch => ch.tagName === container.tagName && ch.className === container.className);
            const grouped = [];
            for (const it of groupItems) {
              const firstLink = (linkSel ? it.querySelector(linkSel) : it.querySelector('a[href]'));
              if (firstLink) {
                const hrefAbs = toAbs(firstLink);
                if (hrefAbs) grouped.push({ href: hrefAbs, text: t(firstLink), titleAttr: firstLink.getAttribute('title') || '' });
              }
            }
            const seen2 = new Set();
            const ded2 = [];
            for (const l of grouped) { if (!seen2.has(l.href)) { seen2.add(l.href); ded2.push(l); } }
            if (ded2.length) {
              dedup = ded2;
    ...
    }, itemsContainerSelector || null, itemLinkSelector || null, url, !!autoScope);
...
app.get('/hover-link-inspector', async (req, res) => {
  ...
  const noProxyParam = String(req.query.noProxy || req.query.proxy || '').toLowerCase();
  const disableProxy = (noProxyParam === '1' || noProxyParam === 'true' || noProxyParam === 'off');
  const sessionId = (req.query.sessionId || '').toString() || Math.random().toString(36).slice(2);
  if (!targetUrl || !isValidUrl(targetUrl)) {
    return res.status(400).json({ success: false, error: 'Provide a valid ?url=' });
  }
  let browser;
  try {
    const launchArgs = [];
    ...
    let usedProxy = false;
    try {
      if (!disableProxy && typeof proxyHost === 'string' && proxyHost && typeof proxyPort !== 'undefined') {
        launchArgs.push(`--proxy-server=http://${proxyHost}:${proxyPort}`);
        usedProxy = true;
      }
    } catch {}
    browser = await puppeteer.launch({ headless: false, args: launchArgs, defaultViewport: null });
    ...
    try {
      if (typeof proxyUser === 'string' && proxyUser && typeof proxyPass === 'string') {
    ...
    if (navError && usedProxy && /tunnel|ERR_TUNNEL|ERR_PROXY/i.test(String(navError && navError.message || ''))) {
      try { await page.close().catch(()=>{}); await browser.close().catch(()=>{}); } catch {}
      // Retry without proxy
      browser = await puppeteer.launch({ headless: false, args: [], defaultViewport: null });
      page = await browser.newPage();
      try { await page.setUserAgent('Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 Chrome/120 Safari/537.36'); } catch {}
      await page.goto(targetUrl, { waitUntil: 'domcontentloaded', timeout: 60000 });
      usedProxy = false;
      navError = null;
    }
    if (navError) throw navError;
    const installInspector = async (frame = page) => { await frame.evaluate((sid) => {
      const makeSelector = (el) => {
        ...
        while (node && node.nodeType === 1 && node !== document.body && node !== document.documentElement) {
          ...
          if (node.id) {
            selector += '#' + (window.CSS && CSS.escape ? CSS.escape(node.id) : node.id.replace(/([ #;.])/g, '\\$1'));
            parts.unshift(selector);
            break;
          }
          const classList = Array.from(node.classList || []);
          if (classList.length) {
            selector += '.' + classList.map(c => (window.CSS && CSS.escape ? CSS.escape(c) : c.replace(/([ #;.])/g, '\\$1'))).join('.');
          }
          let nth = 1, sib = node;
      ...
      btn.addEventListener('click', async (e) => {
        ...
        try {
          await (navigator.clipboard ? navigator.clipboard.writeText(sel) : Promise.reject());
        } catch {
        ...
        const target = lastTarget && lastTarget.closest ? lastTarget.closest('a[href]') : null;
        const data = computeSiblingsFor(target);
        const currentHref = target ? toAbs(target) : '';
        renderSiblings(data, currentHref);
        try {
          const payload = { selector: sel, currentHref, siblings: (data && data.items) || [], pageUrl: location.href, sessionId: sid };
      ...
      const toAbs = (el) => {
        try {
          const href = (el && (el.getAttribute ? el.getAttribute('href') : null)) || (el && el.href) || '';
      ...
      try {
        ...
        if (__hli_raw) {
          ...
          if (Array.isArray(__hli_st.customFields)) customFields = __hli_st.customFields;
        }
      } catch {}
      ...
      if (!ctxMenu) {
        ...
        const renderMenuButtons = () => {
          rowWrap.innerHTML = '';
          const fields = Array.from(new Set(defaultFields.concat(Array.isArray(customFields)?customFields:[])));
          fields.forEach((lab) => rowWrap.appendChild(makeBtn(lab)));
        };
        renderMenuButtons();
        // Dynamic field entry
        const addWrap = document.createElement('div');
        addWrap.style.cssText = 'margin-top:6px; display:flex; gap:6px; align-items:center;';
        const inp = document.createElement('input');
        inp.type = 'text'; inp.placeholder = 'New field label';
        inp.style.cssText = 'background:#111;border:1px solid rgba(255,255,255,0.2);color:#fff;padding:6px 8px;border-radius:4px;flex:1 1 auto;min-width:120px';
        const addBtn = document.createElement('button');
        addBtn.textContent = 'Add Field';
        addBtn.style.cssText = 'background:#5c6bc0;border:0;color:#fff;padding:6px 10px;border-radius:4px;cursor:pointer;flex:0 0 auto';
        const addField = () => {
          ...
          try { if (typeof handleMenuClick === 'function') handleMenuClick(lab); } catch {}
          inp.value='';
        };
        addBtn.addEventListener('click', (e)=>{ e.stopPropagation(); addField(); });
        inp.addEventListener('keydown', (e)=>{ if (e.key==='Enter') { e.stopPropagation(); addField(); }});
        ...
        navWrap.style.cssText = 'margin-top:6px; display:flex; gap:6px; align-items:center;';
        const openBtn = document.createElement('button');
        openBtn.textContent = 'Open Target';
        openBtn.style.cssText = 'background:#2e7d32;border:0;color:#fff;padding:6px 10px;border-radius:4px;cursor:pointer;flex:0 0 auto';
        const copyBtn = document.createElement('button');
        copyBtn.textContent = 'Copy URL';
        copyBtn.style.cssText = 'background:#1976d2;border:0;color:#fff;padding:6px 10px;border-radius:4px;cursor:pointer;flex:0 0 auto';
        const getOpenUrl = (el) => {
          try {
            const abs = (u) => { try { return new URL(u, document.location.href).href; } catch { return ''; } };
            if (!el) {
              // If running inside an iframe document, fallback to its URL
              try { if (window.top !== window) return document.location.href; } catch {}
              return '';
            }
            ...
            if (el.tagName && el.tagName.toLowerCase() === 'iframe') {
              ...
              try { if (el.contentWindow) { const href = el.contentWindow.location && el.contentWindow.location.href; if (href && href !== 'about:blank') return href; } } catch {}
              const dataAttrs = ['data-src','data-lazy-src','data-iframe-src','data-url','data-href'];
              for (const k of dataAttrs) { const v = (el.getAttribute && el.getAttribute(k)) || ''; if (v) { const u = abs(v); if (u) return u; } }
              return '';
            }
            // Anchors: property href or absolute via helper
            if (el.matches && el.matches('a[href]')) return (el.href || toAbs(el));
        ...
        const setBtnState = (btn, enabled) => { btn.disabled = !enabled; btn.style.opacity = enabled ? '1' : '0.5'; btn.style.cursor = enabled ? 'pointer' : 'not-allowed'; };
        const updateNavButtons = (el) => {
          const url = getOpenUrl(el);
          navWrap.setAttribute('data-url', url || '');
          const ok = !!url;
          setBtnState(openBtn, ok); setBtnState(copyBtn, ok);
        };
        openBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          const el = (ctxCurrentEl && document.contains(ctxCurrentEl)) ? ctxCurrentEl : (lastTarget && document.contains(lastTarget) ? lastTarget : null);
          const url = getOpenUrl(el);
          if (url) { try { window.open(url, '_blank', 'noopener'); } catch {} }
          else { try { alert('No hyperlink or iframe URL found for the current target.'); } catch {} }
        });
        copyBtn.addEventListener('click', async (e) => {
          e.stopPropagation();
          const url = navWrap.getAttribute('data-url') || '';
          if (!url) return;
          try { await (navigator.clipboard ? navigator.clipboard.writeText(url) : Promise.reject()); }
          catch {
            const ta = document.createElement('textarea');
        ...
        if (!iframeBtn) {
          ...
          iframeBtn.style.cssText = [
            'position:absolute','z-index:2147483650','background:rgba(20,20,20,0.9)','color:#fff',
            'font:12px -apple-system,BlinkMacSystemFont,Segoe UI,Roboto,Arial',
            'padding:4px 6px','border-radius:4px','border:1px solid rgba(255,255,255,0.2)',
        ...
        const hideIframeButton = () => { try { iframeBtn.style.display = 'none'; iframeTargetEl = null; } catch {} };
        iframeBtn.addEventListener('click', (e) => {
          e.stopPropagation();
          const el = iframeTargetEl;
          if (!el) return;
          try { if (typeof showFor === 'function') showFor(el); } catch {}
          ctxCurrentEl = el;
          try { updateNavButtons && updateNavButtons(el); } catch {}
        ...
        const hideCtx = () => { ctxMenu.style.display = 'none'; try { if (navObserver) { navObserver.disconnect(); navObserver = null; } } catch {} };
        let ctxSelectedEl = null;
        document.addEventListener('contextmenu', (e) => {
          const target = e.target;
          const inPopup = !!(target && ((target === popup) || (popup.contains && popup.contains(target)) || (ctxMenu.contains && ctxMenu.contains(target))));
          if (inPopup) {
            e.preventDefault();
            return;
          }
          ...
          try {
            ...
            if (sel && sel.rangeCount && !sel.isCollapsed) {
              ...
              if (node && node.nodeType === 1) { el = node; ctxSelectedEl = node; }
            }
          } catch {}
          if (!el) el = (lastTarget && document.contains(lastTarget)) ? lastTarget : (target && target.nodeType === 1 ? target : null);
          if (!el) return;
          e.preventDefault();
          // Update popup/selector to reflect the selected element
          try { if (typeof showFor === 'function') showFor(ctxSelectedEl || el); } catch {}
          ctxCurrentEl = el;
          updateNavButtons(el);
          // If target is an iframe, observe lazy src changes to update url state
          try {
            if (el && el.tagName && el.tagName.toLowerCase() === 'iframe') {
              if (navObserver) { try { navObserver.disconnect(); } catch {} }
              navObserver = new MutationObserver(() => updateNavButtons(el));
              navObserver.observe(el, { attributes: true, attributeFilter: ['src','data-src','data-lazy-src','data-iframe-src','data-url','data-href'] });
              // Also poll contentWindow location for same-origin
              let ticks = 0;
              const poll = () => { if (!navObserver) return; try { updateNavButtons(el); } catch {} if (++ticks < 20) setTimeout(poll, 500); };
              setTimeout(poll, 300);
            }
          } catch {}
          placeCtx(e.pageX || (window.scrollX + (el.getBoundingClientRect ? el.getBoundingClientRect().left : 0)), 
                   e.pageY || (window.scrollY + (el.getBoundingClientRect ? el.getBoundingClientRect().bottom : 0)));
        }, true);

        document.addEventListener('click', (e) => {
          const t = e.target;
          if (t && (t === ctxMenu || (ctxMenu.contains && ctxMenu.contains(t)))) return;
          hideCtx();
        }, true);
        ...
        window.addEventListener('keydown', (e) => {
          ...
          try {
            ...
            if (mod && e.shiftKey && (e.key === 'M' || e.key === 'm')) {
              ...
              const el = (ctxCurrentEl && document.contains(ctxCurrentEl)) ? ctxCurrentEl : (lastTarget && document.contains(lastTarget) ? lastTarget : null);
              if (!el) return;
              if (ctxMenu && ctxMenu.style.display === 'block') { hideCtx(); return; }
              try { if (typeof showFor === 'function') showFor(el); } catch {}
              updateNavButtons && updateNavButtons(el);
              const r = el.getBoundingClientRect();
              placeCtx(window.scrollX + r.left, window.scrollY + r.bottom + 8);
            }
          } catch {}
        });
      }
      ...
      const saveState = () => {
        try {
          const st = { columnMap, detailSelectors, siblingsHrefs, baseAnchorSelector, baseAnchorHref, customFields };
      ...
      const loadState = () => {
        try {
          ...
          customFields = Array.isArray(st.customFields) ? st.customFields : customFields;
        } catch {}
      };
      loadState();
      ...
      const ensureTablePanel = () => {
        ...
        btnExtract.style.cssText = [
          'background:#3cb371','border:0','color:#fff','padding:4px 8px','border-radius:4px','cursor:pointer','margin-right:6px'
        ].join(';');
        ...
        btnExtractDetails.style.cssText = [
          'background:#e6a23c','border:0','color:#fff','padding:4px 8px','border-radius:4px','cursor:pointer','margin-right:6px'
        ].join(';');
        const btnExport = document.createElement('button');
        btnExport.textContent = 'Export CSV';
        btnExport.style.cssText = [
          'background:#4a90e2','border:0','color:#fff','padding:4px 8px','border-radius:4px','cursor:pointer','margin-right:6px'
        ].join(';');
        const btnClear = document.createElement('button');
        btnClear.textContent = 'Clear';
        btnClear.style.cssText = [
          'background:#a33','border:0','color:#fff','padding:4px 8px','border-radius:4px','cursor:pointer'
        ].join(';');
        ...
        btnClear.addEventListener('click', () => { extractState = { columns: [], rows: [] }; renderTable(); });
        btnExtract.addEventListener('click', () => { extractAllUsingSiblings(); });
        btnExtractDetails.addEventListener('click', async () => { try { await extractAllDetails(); } catch {} });
        btnExport.addEventListener('click', () => {
          const csv = toCSV(extractState.columns, extractState.rows);
          const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
          const url = URL.createObjectURL(blob);
          const a = document.createElement('a');
          a.href = url; a.download = 'extracted.csv';
      ...
      const renderTable = () => {
        ...
        t.style.cssText = 'border-collapse:collapse; width:100%; table-layout:fixed;';
        const thead = document.createElement('thead');
        const trh = document.createElement('tr');
        extractState.columns.forEach(c => {
          const th = document.createElement('th');
          th.textContent = c;
          th.style.cssText = 'text-align:left; border-bottom:1px solid rgba(255,255,255,0.2); padding:4px; position:sticky; top:0; background:rgba(20,20,20,0.98)';
          trh.appendChild(th);
        });
        ...
        extractState.rows.forEach(r => {
          ...
          extractState.columns.forEach(c => {
            ...
            td.textContent = r[c] || '';
            td.style.cssText = 'padding:4px; border-bottom:1px solid rgba(255,255,255,0.1); word-break:break-word;';
            tr.appendChild(td);
          });
      ...
      const ensureColumn = (name) => {
        if (!extractState.columns.includes(name)) extractState.columns.push(name);
      };
      ...
      const extractTitle = (el) => {
        const a = (el && el.matches && el.matches('a[href]')) ? el : (el && el.querySelector ? el.querySelector('a[href]') : null);
        if (a) {
        ...
        try {
          const h = el && el.querySelector ? el.querySelector('h1,h2,h3,h4,h5,h6') : null;
      ...
      const extractDescription = (el) => {
        ...
        try {
          const descEl = el && el.querySelector ? (el.querySelector('p, .description, [itemprop="description"], meta[name="description"]') || null) : null;
          if (descEl) {
            const txt = (descEl.content || descEl.textContent || '').replace(/\s+/g, ' ').trim();
      ...
      const extractDate = (el) => {
        try {
          const t = (el && el.querySelector ? el.querySelector('time') : null) || (el && el.closest ? el.closest('time') : null);
          const dt = t && (t.getAttribute('datetime') || t.textContent);
          if (dt) return dt.trim();
        } catch {}
        const rxMonth = '(Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:t(?:ember)?)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?)';
        const dateMatchers = [
        ...
        for (const re of dateMatchers) { const m = re.exec(txt); if (m) return m[0]; }
        return '';
      };
      ...
      const handleMenuClick = (label) => {
        ...
        if (label === 'Hyperlink') {
          ...
          const anchorEl = a || (el && el.matches && el.matches('a[href]') ? el : null);
          if (anchorEl && typeof makeSelector === 'function') {
            try { baseAnchorSelector = makeSelector(anchorEl); } catch {}
            try { baseAnchorHref = toAbs(anchorEl); } catch {}
          }
          ...
          try { if (typeof makeSelector === 'function') detailSelectors[label] = makeSelector(el); } catch {}
          val = (el && el.textContent ? el.textContent.trim() : '');
        }
        // For any non-hyperlink field, ensure we persist the selector used
        if (label !== 'Hyperlink') {
          try { if (typeof makeSelector === 'function') detailSelectors[label] = detailSelectors[label] || makeSelector(el); } catch {}
        }
      ...
      const buildRowForElement = (el) => {
        ...
        for (const [label, col] of Object.entries(columnMap)) {
          ...
          let val = '';
          if (label === 'Hyperlink') {
            const a = (el && el.matches && el.matches('a[href]')) ? el : (el && el.querySelector ? el.querySelector('a[href]') : (el && el.closest ? el.closest('a[href]') : null));
            val = a ? toAbs(a) : toAbs(el);
          } else if (label === 'Title') {
            val = extractTitle(el);
          } else if (label === 'Date') {
      ...
      const extractAllUsingSiblings = () => {
        ...
        const anchor = resolveBaseAnchor() || ((lastTarget && lastTarget.closest) ? lastTarget.closest('a[href]') : null);
        ...
        for (const it of items) {
          try {
            ...
            let el = (data && data.elMap && data.elMap[it.href]) ? data.elMap[it.href] : null;
            if (!el) {
              if (data && data.container && it && it.href) {
                const cands = Array.from(data.container.querySelectorAll('a[href]'));
                el = cands.find(a => toAbs(a) === it.href) || null;
              }
            }
            if (!el) { const a = document.createElement('a'); a.href = it.href; a.textContent = it.text || ''; el = a; }
      ...
      const extractAllDetails = async () => {
        try {
          ...
          if (!siblingsHrefs || !siblingsHrefs.length) {
            ...
            try {
              const anchor = (baseAnchorSelector && document.querySelector(baseAnchorSelector)) || null;
              const data = computeSiblingsFor(anchor);
              const items = (data && data.items) || [];
              siblingsHrefs = items.map(it => it && it.href).filter(Boolean);
            } catch {}
          ...
          if (!siblingsHrefs || !siblingsHrefs.length) {
            alert('No siblings recorded. Choose Hyperlink first to capture siblings.');
            return;
          }
          ...
          if (!frame) {
            ...
            frame.style.cssText = 'position:fixed;left:-99999px;top:-99999px;width:800px;height:600px;opacity:0;pointer-events:none;';
            document.body.appendChild(frame);
          }
          const sameOrigin = (u) => { try { return new URL(u, location.href).origin === location.origin; } catch { return false; } };
          const waitLoad = (f) => new Promise((resolve) => {
            const done = () => { f.removeEventListener('load', done); resolve(); };
            f.addEventListener('load', done, { once: true });
          });
          for (const href of siblingsHrefs) {
            try {
              if (!sameOrigin(href)) { // skip cross-origin due to SOP
                const row = {};
                for (const [label, col] of Object.entries(columnMap)) {
                  if (label === 'Hyperlink') row[col] = href; else row[col] = '';
                }
                extractState.rows.push(row); continue;
              }
              frame.src = href;
              await waitLoad(frame);
              const doc = frame.contentDocument;
              const qText = (sel) => {
                try { const el = sel ? doc.querySelector(sel) : null; return (el ? (el.textContent||'').trim() : ''); } catch { return ''; }
              };
              const qDate = (sel) => {
                try {
                  const el = sel ? doc.querySelector(sel) : null;
                  if (!el) return '';
                  const t = el.getAttribute && (el.getAttribute('datetime')||'');
                  if (t) return t.trim();
              ...
              for (const [label, col] of Object.entries(columnMap)) {
                if (label === 'Hyperlink') {
                  ...
                  const sel = detailSelectors['Description'] || 'article, main, .content, .entry-content';
      ...
      const computeSiblingsFor = (anchor) => {
        ...
        while (node && node !== document.body) {
          ...
          const group = Array.from(p.children).filter(ch => sigOf(ch) === sig);
          const items = [];
          ...
          if (ded.length >= 2) {
            if (!best || ded.length > best.items.length) best = { container: node, items: ded, elMap: elMapDed };
          }
          node = p;
        }
        return best || { items: [], container: null, elMap: Object.create(null) };
      };
      ...
      const renderSiblings = (data, currentHref) => {
        ...
        hdr.textContent = items.length ? `Siblings found: ${items.length}` : 'No siblings detected';
      ...
      const place = (rect) => {
        ...
        if (pr.right > vw) left = window.scrollX + rect.left - margin - pr.width;
        ...
        try {
          if (a.tagName && a.tagName.toLowerCase() === 'iframe') {
            ...
            try {
              ...
              originOk = !!src && (new URL(src, document.location.href).origin === document.location.origin);
            } catch {}
            labelText = originOk ? sel : 'Cross-origin iframe — open link in a new tab to map selectors';
          }
        } catch {}
      ...
      document.addEventListener('mouseover', (e) => {
        const target = e.target;
        const inPopup = !!(target && (
          (target === popup) || (popup.contains && popup.contains(target)) ||
          (typeof ctxMenu !== 'undefined' && ctxMenu && ((target === ctxMenu) || (ctxMenu.contains && ctxMenu.contains(target))))
        ));
        if (inPopup) {
      ...
      try { ensureTablePanel(); } catch {}
    }, sessionId); };
    page.on('domcontentloaded', () => installInspector().catch(()=>{}));
    page.on('frameattached', (f) => { try { installInspector(f).catch(()=>{}); } catch {} });
    page.on('framenavigated', (f) => { try { installInspector(f).catch(()=>{}); } catch {} });
    await installInspector();
            // return res.json({ success: true, siblings: finladata });
    return res.json({ success: true, sessionId, message: 'Browser opened. Hover over links to see selector popup.' });
  } catch (e) {
...
app.get('/hover-link-inspector/result', (req, res) => {
  ...
  if (!sessionId) return res.status(400).json({ success: false, error: 'Provide ?sessionId=' });
...
app.post('/hover-link-inspector/report', (req, res) => {
  try {
    ...
    if (!sessionId || !payload || typeof payload !== 'object') {
      return res.status(400).json({ success: false, error: 'Missing sessionId or payload' });
    }
    HoverSessions.set(sessionId, payload);
...
app.get('/hover-link-inspector/embed.js', (req, res) => {
  const sid = (req.query.sessionId || req.query.sid || '').toString() || Math.random().toString(36).slice(2);
